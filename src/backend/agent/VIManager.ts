...
export interface VIGenerateParams {
  brandName: string;
  industry: string;
  style?: string[];
...
export class VIManager {
  private aiProvider: AIProvider;
  private imageGenerator: ImageGenerator;
  private dataPath: string;

  constructor(aiProvider: AIProvider, dataPath: string) {
   ...
   * 检测现有 VI
   */
  async detectVI(files: { name: string; path: string; type: string }[]): Promise<{
    hasVI: boolean;
    colors?: string[];
    logoFound?: boolean;
  }> {
   ...
   * 生成 VI
   */
  async generateVI(params: VIGenerateParams): Promise<VIResult> {
    try {
      // 确定行业类型
   ...
   * 加载 VI
   */
  async loadVI(brandId: string): Promise<BrandVI | null> {
   ...
   * 保存 VI
   */
  private async saveVI(vi: BrandVI): Promise<void> {
   ...
   * 应用 VI 到内容
   */
  applyVIToContent(vi: BrandVI, content: { body: string; title?: string }): {
    styledBody: string;
    styledTitle?: string;
  } {
    // 在内容中注入品牌色彩标识
   ...
   * 生成风格描述
   */
  private async generateStyleDescription(params: VIGenerateParams): Promise<string> {
    const prompt = `
品牌：${params.brandName}
行业：${params.industry}
