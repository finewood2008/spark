...
const MOCK_ITEMS = [
  { id: '1', platform: '小红书', icon: '红', title: '秋冬护肤必备 5 款产品', status: 'published', time: '10分钟前', score: 94 },
  { id: '2', platform: '公众号', icon: '公', title: '品牌故事：从零到一的创业历程', status: 'scheduled', time: '明天 09:00', score: 88 },
  { id: '3', platform: '抖音',   icon: '抖', title: '产品使用教程｜3 分钟学会', status: 'draft', time: '草稿', score: 76 },
  { id: '4', platform: '微博',   icon: '博', title: '双十一大促来了！限时优惠', status: 'failed', time: '1小时前', score: 0 },
];
    ...
    <div className="flex flex-col h-full bg-[#F9FAFB]">
      <div className="page-header">
        <div className="page-header-left">
          <span className="page-header-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></span>
          <div>
        ...
        {filtered.map(item => {
          ...
          return (
            <div key={item.id} className="card group hover:border-[#FF6B35]/30 cursor-pointer transition-all flex items-center p-4 gap-4">
              <div className="w-12 h-12 bg-gray-100 rounded-xl flex items-center justify-center text-sm font-bold text-gray-600 border border-gray-200 shrink-0 group-hover:scale-105 transition-transform">
                {item.icon}
              </div>
              ...
              <div className="flex gap-1 ml-2">
                <button className="p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors text-xs font-medium">编辑</button>
              </div>
            </div>
          );
        })}
